        ...
        <nav className="flex flex-1 flex-col gap-2 p-3">
          {navigation.map(({ to, label, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex h-11 items-center rounded-xl font-bold transition ${collapsed ? 'justify-center px-0' : 'justify-center px-0 lg:justify-start lg:gap-3 lg:px-3'} ${isActive ? 'bg-cyan-400/12 text-fennec-cyan' : 'text-muted hover-surface'}`
              }
            >
      ...
      <nav className="surface fixed inset-x-3 bottom-3 z-30 grid grid-cols-4 rounded-2xl p-1.5 md:hidden">
        {navigation.map(({ to, label, icon: Icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex min-w-0 flex-col items-center gap-1 rounded-xl px-2 py-2 text-[0.68rem] font-bold ${isActive ? 'bg-cyan-400/12 text-fennec-cyan' : 'text-muted'}`
            }
          >
