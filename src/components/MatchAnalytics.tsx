...
/**
 * Presents profile-relative touch and ball telemetry with explicit fallbacks
 * for legacy matches that predate capture analytics.
 */
  ...
  return (
    <section className="space-y-4">
      <div>
        ...
        <h2 className="mt-1 text-xl font-extrabold">Ball analytics</h2>
      </div>
      ...
      <div>
        <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
          <div>
            <h3 className="font-extrabold">Ball touch map</h3>
            <p className="text-muted mt-0.5 text-xs">
              Locations come from exact ball-hit, goal, and crossbar events.
            </p>
          </div>
          <span className="eyebrow">{match.arena || match.playlistName}</span>
        </div>
        <BallTouchMap match={match} profileId={profileId} />
      </div>
    </section>
  );
}
