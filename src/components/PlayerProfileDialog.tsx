...
/**
 * Presents profile-relative player history in a modal, coordinating identity
 * details, match filters, pagination, and dialog dismissal behavior.
 */
export function PlayerProfileDialog({
  ...
  const matches =
    historyQuery.data?.pages.flatMap((page) => page.matches.items) ?? [];
  ...
  return (
    ...
    >
      <div className="surface max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-3xl p-5 sm:p-7">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex min-w-0 items-center gap-3">
              <h2
                ...
                className="truncate text-3xl font-black sm:text-4xl"
              >
                {displayName}
              </h2>
              {identityKind === 'name' && (
                <span className="text-fennec-orange text-xs font-black tracking-wider">
                  BOT
                </span>
              )}
            </div>
            <p className="text-muted mt-2 break-all font-mono text-sm">
              {identityKind === 'name'
                ? 'Name-based identity'
                : (summary?.primaryId ?? playerPrimaryId(playerKey))}
            </p>
          </div>
          <button
            aria-label="Close player profile"
            className="hover-surface flex size-10 shrink-0 items-center justify-center rounded-xl"
            onClick={onClose}
          >
            <X className="size-5" />
          </button>
        ...
        >
          <h3 id="player-history-title" className="text-xl font-extrabold">
            Player history
          </h3>
          {summary && (
            <div className="surface-flat grid grid-cols-2 gap-4 rounded-2xl p-4 sm:grid-cols-4 sm:p-5">
              {[
                ['Together', summary.gamesTogether],
                [
                  'Record together',
                  `${summary.winsTogether}–${summary.lossesTogether}`,
                ],
                ['Opposed', summary.gamesOpposed],
                [
                  'Record against',
                  `${summary.winsAgainst}–${summary.lossesAgainst}`,
                ],
              ].map(([label, value]) => (
                <div key={label}>
          ...
          <div>
            <div className="flex items-center justify-between gap-3">
              ...
              >
                ...
                Filters
                {activeFilterCount > 0 && (
                  <span className="rounded-full bg-cyan-400/15 px-2 py-0.5 text-xs text-fennec-cyan">
                    {activeFilterCount}
                  </span>
                )}
              </button>
            ...
            {filtersOpen && (
              <div
                id="player-history-filters"
                className="surface-flat mt-3 rounded-2xl p-4"
              >
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                  <label>
                    ...
                    >
                      <option value="">Together and against</option>
                  ...
                  <label>
                    ...
                    <select
                      ...
                      value={result}
                      onChange={(event) =>
                        setResult(event.target.value as MatchResultFilter | '')
                      }
                    >
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setPlaylistCategory(
                          event.target.value as PlaylistFilter,
                        )
                      }
                    >
                  ...
                  <label>
                    <span className="eyebrow">Through</span>
                    <input
                      className="control mt-2"
                      type="date"
                      value={to}
          ...
          <div className="space-y-3">
            {historyQuery.isLoading && (
              <div className="surface-flat rounded-2xl p-6">
                Loading history…
              </div>
            )}
            {historyQuery.isError && (
              <div className="surface-flat text-fennec-orange rounded-2xl p-6">
            ...
            {!historyQuery.isLoading &&
              !historyQuery.isError &&
              !matches.length && (
                <div className="surface-flat text-muted rounded-2xl p-6">
