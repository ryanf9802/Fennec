...
export function isWin(match: MatchState, profileId?: string): boolean {
  ...
  return !!player && match.lifecycle === 'completed' && match.winnerTeamNumber === player.teamNumber;
}

export function sessionMetrics(matches: MatchState[], profileId?: string): SessionMetrics {
  const profiles = matches.map((match) => ({ match, player: match.participants.find((item) => item.primaryId === profileId) })).filter((item) => item.player);
  const completed = profiles.filter(({ match }) => match.lifecycle === 'completed');
  const wins = completed.filter(({ match }) => isWin(match, profileId)).length;
  const goalsFor = profiles.reduce((total, { match, player }) => total + (match.teams.find((team) => team.teamNumber === player!.teamNumber)?.score ?? 0), 0);
  const goalsAgainst = profiles.reduce((total, { match, player }) => total + match.teams.filter((team) => team.teamNumber !== player!.teamNumber).reduce((sum, team) => sum + team.score, 0), 0);
  ...
  return {
    ...
    winRate: completed.length ? `${Math.round(wins * 100 / completed.length)}%` : '—',
    ...
    averageScore: profiles.length ? Math.round(profiles.reduce((total, item) => total + item.player!.score, 0) / profiles.length) : 0,
