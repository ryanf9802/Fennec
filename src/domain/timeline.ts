...
/**
 * Flattens nested event payloads into stable dotted paths for cataloging and
 * custom timeline filtering while retaining readable scalar values.
 */
export function flattenPayload(
  ...
  if (Array.isArray(value)) {
    ...
    for (const [key, child] of Object.entries(value))
      flattenPayload(child, prefix ? `${prefix}.${key}` : key, output);
  } else if (prefix) {
...
export function timelineCatalog(
  ...
  return Object.fromEntries(
    Object.entries(result)
      .sort()
      .map(([event, attributes]) => [event, [...attributes].sort()]),
  );
}
...
export function visibleEvents(
  ...
  return match.events
    .filter((event) => {
      ...
      if (settings.timelinePreset === 'curated')
        return curatedEvents.includes(event.eventName);
...
export function eventDetails(
  ...
  let keys: string[];
  if (settings.timelinePreset === 'custom')
    keys = settings.timelineAttributes[event.eventName] ?? [];
  ...
  return keys
    ...
    .map((key) => `${key.split('.').at(-1)}: ${flattened[key]}`)
    .join(' · ');
}
...
function playerFrom(
  value: unknown,
  match: MatchState,
): TimelineMessagePart['player'] | undefined {
...
function playerPart(
  player: TimelineMessagePart['player'] | undefined,
): TimelineMessagePart {
...
function nearby(first: TimelineEvent, second: TimelineEvent): boolean {
  return (
    first.matchClockSeconds === second.matchClockSeconds &&
...
function validGoal(event: TimelineEvent): boolean {
  ...
  return !(
    number(event.payload.GoalSpeed) === 0 &&
    number(event.payload.GoalTime) === 0
  );
}

function relatedGoalEvent(
  event: TimelineEvent,
  ...
  return events.find((candidate) => {
    if (
      candidate.eventName !== 'GoalScored' ||
      !validGoal(candidate) ||
      !nearby(event, candidate)
    )
      return false;
    const name =
      type === 'Assist'
...
/**
 * Converts recognized technical events into human-readable message parts and
 * correlates adjacent goal telemetry when one event lacks complete context.
 */
function semanticParts(
  event: TimelineEvent,
  match: MatchState,
  allEvents: TimelineEvent[],
): TimelineMessagePart[] | undefined {
  ...
  if (event.eventName === 'StatfeedEvent') {
    ...
    if (
      ...
      relatedGoalEvent(event, allEvents)
    )
      return undefined;
    ...
    if (type === 'Shot on Goal')
      return [playerPart(main), { text: ' shot on goal' }];
    if (type === 'Save') return [playerPart(main), { text: ' made a save' }];
    if (type === 'Epic Save')
      return [playerPart(main), { text: ' made an epic save' }];
    if (type === 'Goal') return [playerPart(main), { text: ' scored' }];
    if (type === 'Assist')
      return [playerPart(main), { text: ' assisted a goal' }];
    if (type === 'Demolition' || type === 'Demolish')
      return [
        playerPart(main),
        { text: ' demolished ' },
  ...
  if (event.eventName === 'CrossbarHit') {
    ...
    return [
      playerPart(playerFrom(record(lastTouch?.Player), match)),
      { text: ' hit the crossbar' },
    ];
  }
  if (event.eventName === 'PlayerJoined' || event.eventName === 'PlayerLeft') {
    ...
    return [
      playerPart(player),
      { text: event.eventName === 'PlayerJoined' ? ' joined' : ' left' },
    ];
  }
  if (event.eventName === 'MatchEnded') {
    ...
    return [
      {
        text: winner
          ? `${winner.name || `Team ${winner.teamNumber + 1}`} won the match`
          : 'Match ended',
      },
    ];
  }
...
export function timelineDisplayItems(
  ...
  return events.flatMap((event) => {
    const parts =
      semanticParts(event, match, events) ??
      (settings.timelinePreset === 'curated'
        ? undefined
        : [{ text: words(event.eventName) }]);
    if (!parts) return [];
    const details =
      settings.timelinePreset === 'curated'
        ? undefined
        : eventDetails(event, settings) || undefined;
    return [
      {
        id: event.id,
        ...
        details,
        technicalDetails:
          settings.timelinePreset === 'everything'
            ? JSON.stringify(event.payload, null, 2)
            : undefined,
        technicalDetailsExpired:
          settings.timelinePreset === 'everything' &&
          event.rawPayloadAvailable === false,
      },
    ];
  });
}
