...
const legacyMatch: Omit<MatchState, 'events'> = {
  ...
  participants: [
    {
      name: 'You',
      primaryId: 'Steam|migration-you|0',
      teamNumber: 0,
      score: 100,
      goals: 1,
      assists: 0,
    ...
    {
      name: 'Other',
      primaryId: 'Epic|migration-other|0',
      teamNumber: 1,
      score: 50,
      goals: 0,
      assists: 0,
...
describe('version 2 database migration', () => {
  beforeAll(async () => {
    ...
    await legacy.table('events').put({
      ...
      matchId: 'legacy',
      sequence: 1,
      receivedAt: '2025-01-01T00:02:00Z',
      eventName: 'GoalScored',
      payload: { Scorer: { Name: 'You' }, GoalSpeed: 99, DebugOnly: 'raw' },
    });
    await legacy
      .table('settings')
      .put({ key: 'settings', value: defaultSettings });
    legacy.close();
  });
  afterAll(async () => {
    await db.delete();
  ...
  it('atomically builds normalized history and searchable relationships', async () => {
    ...
    const playerHistory = await historyRepository.getPlayerHistory(
      'id:Steam|migration-you|0',
      'id:Epic|migration-other|0',
    );
    ...
    expect(migrated?.participants.every((player) => player.fifties === 0)).toBe(
      true,
    );
