...
const you: ParticipantState = { name: 'You', primaryId: 'Steam|scale-you|0', teamNumber: 0, score: 100, goals: 0, assists: 0, saves: 0, shots: 1, touches: 10, demos: 0 };
const other: ParticipantState = { name: 'Rival', primaryId: 'Epic|scale-rival|0', teamNumber: 1, score: 100, goals: 0, assists: 0, saves: 0, shots: 1, touches: 10, demos: 0 };

function match(index: number): MatchState {
  const startedAt = new Date(Date.UTC(2020, 0, 1) + index * 600_000).toISOString();
  return {
    id: `scale-${String(index).padStart(5, '0')}`, lifecycle: 'completed', startedAt, lastEventAt: startedAt, endedAt: startedAt,
    playlistId: 11, playlistName: 'Ranked Doubles', playlistCategory: 'ranked', arena: '', timeSeconds: 0, isOvertime: false, isReplay: false,
    winnerTeamNumber: index % 2, teams: [{ teamNumber: 0, name: 'Blue', score: index % 2 ? 1 : 2, colorPrimary: '' }, { teamNumber: 1, name: 'Orange', score: index % 2 ? 2 : 1, colorPrimary: '' }],
    participants: [you, other], events: [],
  };
}
...
describe('25,000-match local archive', () => {
  ...
  it('returns a bounded player page and constant-size all-time summary', async () => {
    await replaceAll(Array.from({ length: 25_000 }, (_, index) => match(index)), defaultSettings);
