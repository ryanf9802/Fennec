...
describe('version 4 player-key migration', () => {
  beforeAll(async () => {
    ...
    await legacy.table('matches').put({
      id: 'legacy-bot', sessionId: 'session', lifecycle: 'completed', startedAt, lastEventAt: startedAt, endedAt: startedAt,
      playlistId: 6, playlistName: 'Private Match', playlistCategory: 'private', arena: '', timeSeconds: 0, isOvertime: false, isReplay: false, winnerTeamNumber: 0,
      teams: [{ teamNumber: 0, name: 'Blue', score: 1, colorPrimary: '' }, { teamNumber: 1, name: 'Orange', score: 0, colorPrimary: '' }],
    });
    const base = { matchId: 'legacy-bot', startedAt, playlistId: 6, playlistCategory: 'private', result: 'win', score: 100, goals: 0, assists: 0, saves: 0, shots: 1, touches: 5, demos: 0 };
    await legacy.table('appearances').bulkPut([
      { ...base, id: 'legacy-bot\u0000id:Steam|you|0', playerKey: 'id:Steam|you|0', name: 'You', primaryId: 'Steam|you|0', shortcut: 1, teamNumber: 0 },
      { ...base, id: 'legacy-bot\u0000id:Unknown|0|0', playerKey: 'id:Unknown|0|0', name: 'Boomer', primaryId: 'Unknown|0|0', shortcut: 2, teamNumber: 1, result: 'loss' },
    ]);
    await legacy.table('events').put({ id: 'legacy-bot:1', matchId: 'legacy-bot', sequence: 1, receivedAt: startedAt, eventName: 'GoalScored', payload: { Scorer: { Name: 'You' } } });
    await legacy.table('rawEvents').put({ id: 'legacy-bot:1', matchId: 'legacy-bot', receivedAt: startedAt, payload: { Scorer: { Name: 'You' }, DebugOnly: 'preserved' } });
  ...
  it('rebuilds persisted appearances and relationships with name identities', async () => {
    ...
    expect(migrated?.participants.map((player) => player.name)).toEqual(['You', 'Boomer']);
    expect(migrated?.events[0]?.payload.DebugOnly).toBe('preserved');
    expect(await historyRepository.getPlayerHistory('id:Steam|you|0', 'name:boomer')).toMatchObject({
      summary: { playerKey: 'name:boomer', identityKind: 'name', latestName: 'Boomer', gamesOpposed: 1 },
    });
  });
});
